...
data_dir = '/fastdata/ggp21rni/wrf_data/cfsr_4km/merged_files/2013_2019/'

wrf_file = data_dir + '2013_2019_merge_temp.nc'
reformat_file = data_dir + 'reformat_temp.nc' 
grid_file = '/fastdata/ggp21rni/wrf_data/cfsr_4km/merged_files/grid.txt'
temp_file = data_dir + 'regrid_temp.nc'

output_file = '/fastdata/ggp21rni/wrf_data/cfsr_4km/merged_files/cfsr_final/CFSR_2013_2019_output.nc'

coord_file = '/fastdata/ggp21rni/wrf_data/cfsr_4km/2019/WRFDS_2019-01-01.nc'

print('Loading in datasets with dask')
df_coord = xr.open_dataset(coord_file)

# Read CFSR file
df = xr.open_dataset(wrf_file).chunk({'Time': 146}) #loading in using dask array chunks
print(df)
print('Generating time coordinates')
#---------Creating and assigning time coordinates----------------------------------------
timestamp = pd.date_range(start="2013-01-01",end="2019-12-31", freq='D')
timestamp = timestamp[(timestamp.day != 29) | (timestamp.month != 2)] # removing leap dates
