...
# Import shell defined model parameters and define them to be sent to other modules
import argparse
import pickle
import numpy as np
import sys

parser = argparse.ArgumentParser(description='COSIPY model parameters')
parser.add_argument('albsnow', help='albedo of snow')
parser.add_argument('albfirn', help='albedo of firn')
parser.add_argument('albice', help='albedo of ice')
parser.add_argument('albtimescale', help='albedo time scaling factor')
parser.add_argument('albdepthscale', help='albedo depth scaling factor')
parser.add_argument('rlengthsnow', help='roughness length of snow')
parser.add_argument('rlengthfirn', help='roughness length of firn')
parser.add_argument('rlengthice', help='roughness length of ice')
parser.add_argument('surface_em', help='Surface emmissivity')
parser.add_argument('mult_rrr', help='Precipitation multiplication factor')
parser.add_argument('model_file', help='model output file')
parser.add_argument('model_input_file', help='model intput file')

args = parser.parse_args(sys.argv[1:])
    
albsnow = args.albsnow
albfirn = args.albfirn
albice = args.albice
albtime = args.albtimescale
albdepth = args.albdepthscale
rsnow = args.rlengthsnow
rfirn = args.rlengthfirn
rice = args.rlengthice
surf_em = args.surface_em
mult_rrr = args.mult_rrr
model_file = args.model_file
model_input_file = args.model_input_file

param_array = np.array([albsnow, albfirn, albice, albtime, albdepth, rsnow, rfirn, rice, surf_em, mult_rrr])

param_file = open('param_values', 'wb')
pickle.dump(param_array, param_file)
param_file.close()

model_file_file = open('model_file_file', 'wb')
pickle.dump(model_file, model_file_file)
model_file_file.close() 

model_input_file_file = open('model_input_file_file', 'wb')
pickle.dump(model_input_file, model_input_file_file)
model_input_file_file.close() 


#print(albsnow,albfirn,albice,albtime,albdepth,rsnow,rfirn,rice,model_file)
...
def run_cosipy(cluster, IO, DATA, RESULT, RESTART, futures):
    ...
    with Client(cluster) as client:
        ...
        if stake_evaluation is True:
            ...
            df_stakes_loc = pd.read_csv(stakes_loc_file, na_values='-9999')
            df_stakes_data = pd.read_csv(stakes_data_file, delimiter=',',index_col='TIMESTAMP')
            df_stakes_data.index = pd.to_datetime(df_stakes_data.index).strftime('%Y-%m-%d')
